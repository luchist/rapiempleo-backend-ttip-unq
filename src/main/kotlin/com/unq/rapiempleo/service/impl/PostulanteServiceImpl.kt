...
class PostulanteServiceImpl (
    ...
    override fun notificarAccionEnCv(avisoPostulacion: AvisoPostulanteDTO) {
        ...
        if (postulacion!!.estadoCv == EstadoCvPostulado.ESPERA
            && avisoPostulacion.tipo_aviso == EstadoCvPostulado.VISTO)
        {
            postulacion.estadoCv = EstadoCvPostulado.VISTO
            postulanteANotificar.notificacionesCv.add(
                NotificationEntry(avisoPostulacion.tipo_aviso,ofertaPostulada.titulo)
            )
    ...
    override fun removerCvIndicado(cvEntryReq : CvEntryRequestDTO) {
        val postulante = postulanteRepository.findById(cvEntryReq.idPostulante)
            .orElseThrow { throw PostulanteNotFoundException() }
        val checkCVStored = postulante.cvEntries.find { cv -> cv.cvPath == cvEntryReq.cvPath }
        if (checkCVStored == null) {
            throw CvNotFoundException()
        }
